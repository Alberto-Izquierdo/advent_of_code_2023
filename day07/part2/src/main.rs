...
impl HandAndBet {
    fn new(hand: String, bet: i32) -> Self {
        ...
        let hand_type = map.iter().fold(
            HandType::HighCard,
            |current_hand_type, (character, amount)| {
                if character != &'J' {
                    match amount {
        ...
        let hand_type = apply_jokers(hand_type, *jokers);
        HandAndBet {
            hand: CamelHand {
                hand: hand_vec,
                hand_type,
...
fn improve_hand(hand_type: HandType) -> HandType {
    match hand_type {
...
fn apply_jokers(hand_type: HandType, jokers: i32) -> HandType {
    let mut hand_type = hand_type;
    for _ in 0..jokers {
        hand_type = improve_hand(hand_type);
    }
    hand_type
}
