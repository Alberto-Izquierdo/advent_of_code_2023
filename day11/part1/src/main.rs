        ...
        .map(|(x, y)| {
            (
                *x + empty_columns
                    ...
                    .count(),
                *y + empty_rows.iter().take_while(|row| *row < y).count(),
            )
        })
        .collect()
}
...
fn find_empty_rows(map: &Vec<(usize, usize)>) -> Vec<usize> {
    let height = map.iter().max_by(|(_, y1), (_, y2)| y1.cmp(y2)).unwrap().1;
    let result = (0..height)
...
fn find_empty_columns(map: &Vec<(usize, usize)>) -> Vec<usize> {
    let width = map.iter().max_by(|(x1, _), (x2, _)| x2.cmp(x1)).unwrap().1;
    let result = (0..width)
